...
CREATE OR REPLACE VIEW  v_item AS 
 SELECT 
	...
	item.item_active::character varying(255) AS active, 
   ...
   FROM public.item
   LEFT JOIN public.prodcat ON item.item_prodcat_id = prodcat.prodcat_id, classcode, uom i, uom p
   WHERE 
...
ALTER TABLE  v_item
  OWNER TO admin;
GRANT ALL ON TABLE  v_item TO admin;
GRANT SELECT ON TABLE  v_item TO public;
GRANT SELECT, UPDATE, INSERT, REFERENCES, TRIGGER ON TABLE  v_item TO xtrole;
COMMENT ON VIEW  v_item
  IS 'Orbit 803 StepLogic view V_ITEM uses this view';
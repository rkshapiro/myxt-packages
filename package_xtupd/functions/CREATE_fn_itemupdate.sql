...
DECLARE
 ...
 (
  SELECT itemupdate_number, itemupdate_descrip1, itemupdate_descrip2, 
       itemupdate_classcode_code, 
       -- replace the bar that replaced the \n in the export view back
       -- to match what is in the database. Work around for Import data utility error %4
       replace(trim(both from itemupdate_comments),' | ',E'\n') AS itemupdate_comments, 
       CASE 
       ...
       itemupdate_listprice, itemupdate_listcost, 
       replace(trim(both from itemupdate_extdescrip),' | ',E'\n') AS itemupdate_extdescrip, itemupdate_maxcost, 
       itemupdate_lastupdated,item_id,
       item_descrip1, item_descrip2, 
       item_classcode_id, 
       trim(both from item_comments) AS item_comments, 
       item_sold, 
       item_prodweight, item_packweight, item_prodcat_id, 
       item_listprice, item_listcost,
       trim(both from item_extdescrip) AS item_extdescrip, item_maxcost

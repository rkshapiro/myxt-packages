...
CREATE OR REPLACE VIEW _report.sales_revenue AS 
 SELECT 'S'::text AS source,
  ...
  WHERE
        ...
        END > 0::numeric AND coitem.coitem_status <> 'X'::bpchar AND NOT (custinfo.cust_id = ANY (ARRAY[1476, 6562, 6574, 6959, 9154, 9150, 9152, 9154])) AND NOT upper(cohead.cohead_custponumber) ~ 'INVALID'::text AND COALESCE(coitem.coitem_promdate, coitem.coitem_scheddate) > '2012-06-30'::date
  GROUP BY custinfo.cust_id, custinfo.cust_name, custinfo.cust_number, cohead.cohead_number, prj.prj_number, prodcat.prodcat_code, custtype.custtype_code, cohead.cohead_orderdate, cohead.cohead_id, coitem.coitem_promdate, item.item_number, item.item_descrip1, crm.crmacct_owner_username
UNION
 SELECT 'I'::text AS source,
    ...
    sum(invcitem.invcitem_ordered) AS qtyord,
    sum(invcitem.invcitem_price * invcitem.invcitem_ordered) AS extprice,
    COALESCE(prj.prj_number, 'blank'::text) AS prj_number,
  ...
  WHERE invcitem.invcitem_price > 0::numeric AND NOT (custinfo.cust_id = ANY (ARRAY[1476, 6562, 6574, 6959, 9154, 9150, 9152, 9154])) AND NOT upper(invchead.invchead_ponumber) ~ 'INVALID'::text AND invcitem.invcitem_coitem_id IS NULL AND invchead.invchead_invcdate > '2012-06-30'::date
...
ALTER TABLE _report.sales_revenue
  OWNER TO admin;
GRANT ALL ON TABLE _report.sales_revenue TO admin;
GRANT ALL ON TABLE _report.sales_revenue TO xtrole;
GRANT SELECT, REFERENCES ON TABLE _report.sales_revenue TO public;
COMMENT ON VIEW _report.sales_revenue
  IS 'used by executive dashboard';

...
CREATE OR REPLACE FUNCTION _custom.araging(date, boolean, boolean)
  RETURNS SETOF araging AS
$BODY$
-- Copyright (c) 1999-2014 by OpenMFG LLC, d/b/a xTuple. 
-- See www.xtuple.com/CPAL for the full text of the software license.
-- 20140907:rks changed to use docdate for threshold testing rather than duedate for Orbit 234
-- 20150112:rek Orbit 796 - port to 4.7 - use aropen_docdate instead of aropen_duedate.
DECLARE
...
BEGIN
  ...
  FOR _x IN
        ...
        --today and greater:
        CASE WHEN((aropen_docdate >= DATE(_asOfDate))) THEN balance
             ELSE 0.0 END AS cur_val,

        --0 to 30
        CASE WHEN((aropen_docdate >= DATE(_asOfDate)-30) AND (aropen_docdate < DATE(_asOfDate))) THEN balance
             ELSE 0.0 END AS thirty_val,

        --30-60
        CASE WHEN((aropen_docdate >= DATE(_asOfDate)-60) AND (aropen_docdate < DATE(_asOfDate) - 30 )) THEN balance
             ELSE 0.0 END AS sixty_val,

        --60-90
        CASE WHEN((aropen_docdate >= DATE(_asOfDate)-90) AND (aropen_docdate < DATE(_asOfDate) - 60)) THEN balance
             ELSE 0.0 END AS ninety_val,

        --greater than 90:
        CASE WHEN((aropen_docdate > DATE(_asOfDate)-10000) AND (aropen_docdate < DATE(_asOfDate) - 90)) THEN balance
             ELSE 0.0 END AS plus_val,

        --total amount:
        CASE WHEN((aropen_docdate > DATE(_asOfDate)-10000)) THEN balance
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION _custom.araging(date, boolean, boolean)
  OWNER TO admin;
GRANT EXECUTE ON FUNCTION _custom.araging(date, boolean, boolean) TO public;
GRANT EXECUTE ON FUNCTION _custom.araging(date, boolean, boolean) TO admin;
GRANT EXECUTE ON FUNCTION _custom.araging(date, boolean, boolean) TO xtrole;
